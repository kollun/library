...
//if(isset($arParams["ID"]) && empty($arParams["ID"])) $arParams["ID"] = 0;
...
//pr($arGroups);
//echo CIBlock::GetPermission($arParams["IBLOCK_ID"]);
// check whether current user can have access to add/edit elements
$bAllowAccess = (CIBlock::GetPermission($arParams["IBLOCK_ID"]) >='W' );
//$bAllowAccess = (); 
/*if ($arParams["ID"] == 0)
{
...
if ($bAllowAccess)
{
	...
	elseif($arParams["ELEMENT_ASSOC"] == 'NONE')
	{
	...
	if ($arParams["ID"] > 0)
	{
		...
		if ($arElement = $rsIBlockElements->GetNext(true, false))
		{
			$bAllowAccess = true;
			...
			if ($bWorkflowIncluded)
			{
				...
				if ($LAST_ID != $arElement["ID"])
				{
					...
					$arElement = $rsElement->GetNext(true, false);
				}
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
			...
			else
			{
				...
				else
				{
					if($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "F")
					{
						...
						if($_REQUEST["MAKE_IMG_FROM_FILE"][$propertyID]["MAKE"] == 'Y' && $_REQUEST["MAKE_IMG_FROM_FILE"][$propertyID]['SOURCE_FILE_PROPERTY'] > 0 )
						{
							
							$prop_id = $_REQUEST["MAKE_IMG_FROM_FILE"][$propertyID]['SOURCE_FILE_PROPERTY'];
							$pageNum = 0;
							if( $_REQUEST["MAKE_IMG_FROM_FILE"][$propertyID]['PAGE'] >0 ) $pageNum = $_REQUEST["MAKE_IMG_FROM_FILE"][$propertyID]['PAGE'] - 1;
							$arSourceProp = each($_REQUEST['PROPERTY'][$prop_id]);
							//pr($arSourceProp);
							
							
							
							if($_FILES['PROPERTY_FILE_'.$prop_id.'_'.$arSourceProp['key']]['tmp_name'] != ''){
								$arFile = $_FILES['PROPERTY_FILE_'.$prop_id.'_'.$arSourceProp['key']];
							}
							else{
								$arFile = CFile::MakeFileArray($arSourceProp['value']);
							}	
							//pr($arFile);
							...
							$image = new Imagick($arFile['tmp_name'].'['.$pageNum.']'); //[0] indicate the number of the wanted page
							//$image->setResolution( 300, 300 );
							$image->setImageFormat( "jpg" );
							$arIMAGE=Array(
								"name" => str_replace('pdf', 'jpg',$arFile['name']),
							//	"size" => "размер",
							...
							$fid = CFile::SaveFile($arIMAGE, "iblock/images");
							if($fid > 0 ) 
							{
								$arUpdateValues[$propertyID] = CFile::MakeFileArray($fid);
								//pr(CFile::MakeFileArray($fid));
							}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if(!isset($arUpdateValues['CODE']) || $arUpdateValues['CODE']) $arUpdateValues['CODE'] =  Cutil::translit($arUpdateValues['NAME'],"ru",array("replace_space"=>"-","replace_other"=>"-"));
