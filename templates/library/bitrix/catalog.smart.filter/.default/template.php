...
<div class="bx-filter <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "bx-filter-horizontal"?>">
	<div class="bx-filter-section container-fluid">
		...
		<div class="row">
			<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-title"><?echo "Раздел"?></div>
		</div>	
		<div class="row">
			<ul class="bx-disk-wood-folder">
			
				<?$cur_depth = 1;?>
				<?foreach($arResult['SECTIONS'] as $arSect):?>
				<?//if($cur_depth > $arSect['DEPTH_LEVEL']){
					$count_depth = $cur_depth - $arSect['DEPTH_LEVEL'];
					for($i = 0; $i < $count_depth; $i++) echo "</li></ul>";	
				...
				<li class = "section_id_<?=$arSect['ID']?> bx-disk-folder-container bx-disk-parent bx-disk-loaded <?if($arSect['ACTIVE'] != 'Y') echo "bx-disk-close"?>">
					<div class="bx-disk-folder-container">
							<table>
							<tr>
								<td class="bx-disk-wf-arrow"><?if($arSect['SUBSECTIONS'] == "Y") echo "<span data-section_id='".$arSect['ID']."'></span>";?></td>
								<td class="bx-disk-wf-folder-icon"><span></span></td>
								<td class="bx-disk-wf-folder-name"><span <?if($arResult['SECTION']['ID'] == $arSect['ID']) echo "class = 'active'";?>><a href = "<?=$arSect['SECTION_PAGE_URL']?>"><?=$arSect['NAME']?></a></span></td>
							</tr>
							</table>
					</div>
					
					
					<?if($arSect['SUBSECTIONS'] == "Y"){?>
						<ul class="bx-disk-wood-folder">
					<?} 
					$cur_depth = $arSect['DEPTH_LEVEL'];
					?>
		...
		<script>
			$('.bx-filter .bx-disk-folder-container .bx-disk-wf-arrow span').click(function(){
				$(".bx-disk-wood-folder .section_id_"+$(this).data('section_id')).toggleClass('bx-disk-close');
				//alert($(this).data('section_id'));
