...
$INTRANET_TOOLBAR->AddButton(array( 
	...
	$INTRANET_TOOLBAR->AddButton(array( 
		'HREF' => '/bitrix/admin/iblock_list_admin.php?IBLOCK_ID='.$arParams['IBLOCK_ID'].'&type='.$arParams['IBLOCK_TYPE'], 
		"TEXT" => 'Элементы', 
		"ICON" => '', 
		"SORT" => 1000, 
	)); 	
...
{?>
	<ul class="library_section_items">
	<?
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if($arItem['PROPERTIES']['FILE_SOURCE']['VALUE']!= '')
			$dounload_url = CFile::GetPath($arItem['PROPERTIES']['FILE_SOURCE']['VALUE']);
		elseif($arItem['PROPERTIES']['FILE_VIEW']['VALUE']!= '')
			$dounload_url = CFile::GetPath($arItem['PROPERTIES']['FILE_VIEW']['VALUE']);

	?>
			<li class="library_section_item_wrap"  id="<? echo $strMainID; ?>">
				<div class="library_section_item_body">
					<div class="library_section_image_wrap">
						<?if($dounload_url):?>
							<div class = 'btn_save_wrap'>
								<a class = "btn_save" href = "<?=$dounload_url?>" target = "_blank">Скачать</a>
							</div>
						<?endif?>
						<?//echo pr($arItem);?>
						<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class = "cover">
							<?if(empty($arItem['PREVIEW_PICTURE']['SRC'])):?>
								<div class = 'no_image_cover'>
									<div class = 'no_image_cover_author'>
										<?if(is_array($arItem['DISPLAY_PROPERTIES']['AUTHOR']['DISPLAY_VALUE'])) echo strip_tags(implode(", ",$arItem['DISPLAY_PROPERTIES']['AUTHOR']['DISPLAY_VALUE'])); else echo strip_tags($arItem['DISPLAY_PROPERTIES']['AUTHOR']['DISPLAY_VALUE'])?>
									</div>
									<div class = 'no_image_cover_title'>
										<?=$arItem['NAME']?>
									</div>
									<div class = 'no_image_cover_year'>
										<?=$arItem['DISPLAY_PROPERTIES']['YEAR_PUBLICATION']['DISPLAY_VALUE']?>
									</div>
								</div>
							<?else:?>
					...
					<div class="library_section_info_wrap">
						<a href="<?=$arItem['DETAIL_PAGE_URL']?>"><h2><?=$arItem['NAME']?></h2></a>
							<?foreach($arItem['DISPLAY_PROPERTIES'] as $arProp):?>
								<?if($arProp['DISPLAY_VALUE'] == '') continue;?>
								<div class="library_section_item_prop">
										<span class="name"><?=$arProp['NAME']?>:</span> <?if(is_array($arProp['DISPLAY_VALUE'])) echo implode(" / ",$arProp['DISPLAY_VALUE']); else echo $arProp['DISPLAY_VALUE']?>
								</div>
						...
						<!--	
								<span class="name">Отзывы</span> <div class="bx_stars_container">
									<div class="bx_stars_bg"></div>
									<div id="result_stars" class="bx_stars_progres" style="width: 0%;"></div>
								</div>
								<span>(0)</span> 
							</div>
							</div>-->
